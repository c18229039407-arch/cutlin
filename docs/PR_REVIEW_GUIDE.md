...
Cutlin pull requests. It is a review framework, not a closed checklist.
Use it to structure the review, then keep looking for issues that are specific
to the PR in front of you.

Cutlin is an agent-orchestrated video production system. Many PRs affect
...
- Does this PR move Cutlin in the right direction?
...
Check whether the PR solves a real Cutlin problem.
...
Cutlin works with video, audio, image generation, and local models. Small
...
## Cutlin Architecture Checks

### Agent-First Architecture

Cutlin's control plane is the agent following markdown skills and YAML
...
Cutlin uses checkpoints for resume, human approval, audit trails, and
stage gating.
...
Cutlin can compose with Remotion, HyperFrames, or FFmpeg. Runtime choices
...
Cutlin should not silently spend money or imply a paid provider is free.
...
- Is this provider useful for Cutlin workflows?
...
- Is the provider internationally known, nationally or regionally important,
  or otherwise clearly valuable to Cutlin users?
...
Provider viability matters. Cutlin should not become a grab bag of
