<p align   对齐   对齐="center"   "center"   "center">   <p align   对齐   对齐="center"   "center"   "center">
  <img src="assets/logo.png"   "assets/logo.png" alt="Cutlin" width="200"><img src="assets/logo.png"   "assets/logo.png"   "assets/logo.png" alt="Cutlin" width="200">
</   & lt;p>   < / p></ & lt; <；

<h1 align   对齐="center"   "center">Cutlin</   & lt;h1><h1 align   对齐="center"   "center">Cutlin</   & lt;h1><h1 align   对齐   对齐="center"   "center"   "center">Cutlin</   & lt;h1><h1 align   对齐   对齐="center"   "center"   "center">Cutlin</   & lt;h1>

<p align   对齐="center"   "center">简体中文 · <a href="README_en.md"   "README_en.md">English</   & lt;a></   & lt;p>< p align   对齐=" center"   " center" > Simplified Chinese · < a href=" README_en.md"   “README_en.md   “ README_en.md" > English< /a> < /p><p align   对齐   对齐="center"   "center"   "center">简体中文 · <a href="README_en.md"   "README_en.md"   "README_en.md">English</   & lt;a></   & lt;p>< p align   对齐   对齐=" center"   " center"   " center" > Simplified Chinese · < a href=" README_en.md"   “README_en.md   “ README_en.md"   “README_en.md   “ README_en.md" > English< /a> < /p>
...
装四样东西：**Python 3.10+   Python 3.10**（[python.org](https://www.python.org/downloads/)）、**FFmpeg**（macOS `brew install ffmpeg   Brew安装ffmpeg`，Ubuntu `sudo apt install ffmpeg`）、**Node.js 18+   Node.js 18**（[nodejs.org](https://nodejs.org/   https:/安装四样东西：**Python 3.10**（[python.org](https://www.python.org/downloads/)）、**FFmpeg**（macOS 使用 `brew install ffmpeg   Brew安装ffmpeg` 安装，Ubuntu 使用 `sudo apt install ffmpeg`）、**Node.js 18**（[nodejs.org](https:/
odejs.org/)）
odejs.org/)），以及任意一款 **AI 编程助手**（Claude Code / Cursor / Copilot / Windsurf / Codex 都行）。Install four items: **Python 3.10** ([python.org](https://www.python.org/downloads/)), **FFmpeg** (macOS: `brew install ffmpeg   Brew安装ffmpeg`, Ubuntu: `sudo apt install ffmpeg`), **Node.js 18** ([nodejs.org](https:/odejs.org/))，以及任意一款 **AI 编程助手**（Claude Code / Cursor / Copilot / Windsurf / Codex 均可）。安装以下四项：**Python 3.10** ([python.org](https://www.python.org/downloads/))、**FFmpeg**（macOS：`brew install ffmpeg   Brew安装ffmpeg`，Ubuntu：`sudo apt install ffmpeg`）、**Node.js 18** ([nodejs.org](https:/Install the following four items: **Python 3.10** ([python.org](https://www.python.org/downloads/)), **FFmpeg** (macOS: `brew install ffmpeg   Brew安装ffmpeg`, Ubuntu: `sudo apt install ffmpeg`), **Node.js 18** ([nodejs.org](https:/
...
odejs.org/)), and any **AI coding assistant** (Claude Code / Cursor / Copilot / Windsurf / Codex are all acceptable).odejs.org/），以及任何**AI编程助手**（Claude Code / Cursor / Copilot / Windsurf / Codex 均可接受）。odejs.org/), and any **AI coding assistant   AI编码助手** (Claude Code / Cursor / Copilot / Windsurf / Codex are all acceptable).
...
> ```bash   比;”“bash   比;” &rdquo
> python3 -m venv .venv && source .venv/bin/activate   # 建好并激活虚拟环境> python3 -m venv .venv & & source .venv/bin/activate   # Create and activate the virtual environment> python3 -m venv .venv && source .venv/bin/activate   # 创建并激活虚拟环境
> python -m pip install -r requirements.txt            # Python 依赖
> ( cd remotion-composer && npm install )              # 渲染引擎依赖> (cd remotion-composer && npm install)              # Rendering engine dependencies> (cd remotion-composer && npm install)              # 渲染引擎依赖
> python -m pip install piper-tts                      # 免费本地 TTS> python -m pip install piper-tts                      # 免费本地语音合成> python -m pip install piper-tts  # Free local TTS
> cp .env.example .env                                 # 密钥配置模板> cp .env.example .env                                 # Key configuration template> cp .env.example .env                                 # Key configuration template cp .env.example .env                                 # Key configuration template
> ```   比;' ' '
...
各平台入口文件已备好：Claude Code 看 `CLAUDE.md`，Cursor 看 `CURSOR.md` 与 `.cursor/rules/`，GitHub Copilot 看 `COPILOT.md` 与 `.github/copilot-instructions.md`，Codex 看 `CODEX.md`，Windsurf 看 `.windsurfrules`——殊途同归，最终都指向 `AGENT_GUIDE.md`（操作契约）与 `PROJECT_CONTEXT.md`（架构地图）两份母文档。The entry files for each platform are ready: for Claude Code, see `CLAUDE.md`; for Cursor, see `CURSOR.md` and `.cursor/rules/   .cursor /规则/`; for GitHub Copilot, see `COPILOT.md` and `.github/copilot-instructions.md`; for Codex, see `CODEX.md`; for Windsurf, see `.windsurfrules`. — Different paths lead to the same destination—ultimately pointing to two core documents: `AGENT_GUIDE.md` (the operational contract) and `PROJECT_CONTEXT.md` (the architecture map).
...
深挖资料：技术全景在 `docs/ARCHITECTURE.md`，供应商注册/价格/额度在 `docs/PROVIDERS.md`，智能体契约在 `AGENT_GUIDE.md`。Deep dive into the documentation: technical overview in `docs/ARCHITECTURE.md`, provider registration/pricing/quota in `docs/PROVIDERS.md   文档/ PROVIDERS.md`, and agent contracts in `AGENT_GUIDE.md`.

**测试**：

```bash   ”“bash
# 契约测试套件——一个 API Key 都不配也能全绿
make test-contracts   使test-contracts

# 全量测试
make test   做测试
```

**反馈**：Bug、功能建议和工作流讨论统一走 [GitHub Issues](https://github.com/c18229039407-arch/cutlin/issues)。**Feedback   反馈**: Bugs, feature suggestions, and workflow discussions should all be submitted via [GitHub Issues   GitHub的问题](https://github.com/c18229039407-arch/cutlin/issues).
