...
**Scored selection.** Every vendor pick gets scored, and the losers' scores are filed too. Image, voice and music run the generic 7-axis engine: task fit 30%, output quality 20%, control 15%, reliability 15%, cost efficiency 10%, latency and continuity 5% each. Video runs its own 8-axis scene-weighted engine — prompt following, motion stability, visual quality, physical accuracy, subject consistency, controllability, audio capability, engineering — with weights that switch per scene: an e-commerce ad puts subject consistency and engineering at 25% each, a brand TVC puts controllability and visual quality at 20% each, a documentary pushes physical accuracy to 20%. Baseline scores carry confidence and procurement-status flags, so a provider you cannot actually buy will not win even with the highest raw score. Losers' scores are archived beside the winner's. The selector also normalizes loose briefs into scoreable intent — even a single line like "Pixar-style short with consistent characters" — and its output carries the winner's `agent_skills` index so the agent can read the matching Tier-3 pack before writing a single generation prompt.
