"""LTX-2 (Lightricks) video generation via fal.ai's hosted API.

The fastest, cheapest iteration tier on the cloud path — ideal for
concepting loops before committing budget to a premium model. Complements
the local `ltx_video_local` and self-hosted `ltx_video_modal` channels.
"""
...
class Ltx2VideoFal(BaseTool):
    ...
    best_for = [
        "ultra-cheap, fast concepting loops (draft-tier motion)",
        "high-volume A/B iteration on prompts before premium renders",
        "Apache-2.0 model family, consistent with the local LTX channels",
    ]
    not_good_for = ["final hero shots", "complex multi-subject scenes"]
    fallback_tools = ["ltx_video_local", "ltx_video_modal", "wan_video_fal"]
    ...
    input_schema = {
        ...
        "properties": {
            ...
            "model_variant": {
                ...
                "enum": ["ltx-2"],
                "default": "ltx-2",
                "description": "LTX endpoint family on fal.",
            },
            ...
            "output_path": {"type": "string", "default": "ltx2_fal_output.mp4"},
        },
        "required": ["prompt"],
    }
    ...
    def estimate_cost(self, inputs: dict[str, Any]) -> float:
        # fal prices LTX-2 per megapixel of video (~$0.0018/MP); a short 720p clip lands well under $0.25.
        return 0.20
    ...
    def execute(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        variant = inputs.get("model_variant", "ltx-2")
        model_path = f"{variant}/{operation.replace('_', '-')}"
        ...
        if not ok:
            return ToolResult(success=False, error=f"LTX-2 (fal) generation failed: {info}")
