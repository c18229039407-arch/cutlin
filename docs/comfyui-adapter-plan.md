# ComfyUI Provider Adapter for Cutlin
...
Cutlin's local GPU tools (`wan_video`, `hunyuan_video`, `cogvideo_video`,
...
A ComfyUI adapter gives Cutlin access to any model ComfyUI supports,
...
**Future path:** ACE-Step support should be revisited once Cutlin decides
...
URL when Cutlin knows the canonical source. Agents should surface that
...
using Cutlin's 7-dimension scoring:
...
Cutlin should treat those as custom workflow profiles until a blessed
...
### Future (add models to ComfyUI, no code changes to Cutlin)
...
4. **Music generation:** ACE-Step works in ComfyUI but Cutlin needs a
