...
This is the **Cutlin-specific** guide to HyperFrames. It explains when
Cutlin pipelines should choose HyperFrames over Remotion, how Cutlin
...
## When Cutlin should pick HyperFrames (vs Remotion vs FFmpeg)

Cutlin separates two concepts:
...
| Synthetic UI / fake terminal / fake browser demo | Either — depends on existing coverage | Cutlin already ships Remotion `TerminalScene` (see `synthetic-screen-recording` Layer 3). For UI chrome beyond terminal, HyperFrames HTML is easier. |
...
Cutlin artifacts into HyperFrames project files:

| Cutlin artifact field | HyperFrames target |
|---|---|
...
`STORYBOARD.md` as step-by-step workspace files. Cutlin does **not**
...
Cutlin playbooks currently translate into Remotion `themeConfig`
...
- ❌ Running `hyperframes init` from the Cutlin orchestrator. `init`
