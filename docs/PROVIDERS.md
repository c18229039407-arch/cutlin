# Cutlin Provider Guide

Everything you need to know about every provider in Cutlin — setup instructions, pricing, free tiers, and what each unlocks.
...
Cutlin now uses those published rates in the Grok tool estimators.
...
Cutlin uses the new-console API key flow:
...
Doubao Speech 2.0 is billed by character package or usage in Volcengine. Cutlin estimates cost from text length and prefers provider-returned usage metadata when available.
...
The CLI is consumed as `npx hyperframes`. Do not use `npx @hyperframes/cli`; that package name is not the Cutlin runtime path.
...
HyperFrames workspaces live under `projects/<project-name>/hyperframes/`. Final videos still follow the normal Cutlin convention: `projects/<project-name>/renders/final.mp4`.
