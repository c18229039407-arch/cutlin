...
> **系统里没有 `make`？** 按平台手动执行等价命令——macOS/Linux：`python3 -m venv .venv && source .venv/bin/activate && python -m pip install -r requirements.txt && cd remotion-composer && npm install && cd .. && python -m pip install piper-tts && cp .env.example .env`
>
...
- **图生动画：** Piper 念稿，图像出画面，Remotion 把它们编排成有动感的成片。
- **本地角色动画：** SVG 绑定 + 姿势库 + GSAP 时间线，HyperFrames 渲染卡通角色的表演，产物落在 `projects/<project-name>/renders/final.mp4`。
- **真素材成片：** 纪录片蒙太奇流水线从 Archive.org、NASA、Wikimedia Commons（可选叠加 Pexels、Unsplash）构建支持 CLIP 检索的语料库，把真实动态影像剪成一部完整的片子。
...
- **12 条生产流水线** — 解说、口播、录屏演示、电影预告、动画、播客、本地化、纪录片蒙太奇，全覆盖
- **52 种生产工具** — 视频生成、图像创建、TTS、音乐、混音、字幕、画面增强、内容分析
- **400 多项智能体技能** — 制作技能、流水线导演、创意方法、质检清单和深度技术知识包，教会智能体像行家一样用好每件工具
- **参考驱动的创作** — 丢一条你喜欢的视频进来，智能体把它转译成扎实且差异化的制作方案，你不必憋完美提示词
- **不买视频模型也能做真纪录片** — 从免费/开源的动态影像和档案里剪出真正的片子，而不是给图片加推拉镜头
- **内置实时联网调研** — 写稿前对 YouTube、Reddit、新闻和学术站做 15-25 轮检索，确保内容立足于最新的真实数据
- **免费/本地与云端供应商并重** — 每项能力都有开源本地方案或高级 API 两条路，有什么用什么
- **没有供应商锁定** — 供应商随时可换。系统按 7 维（任务契合、输出质量、控制能力、可靠性、成本效益、延迟、连续性）打分并自动选最优
- **生产级质量关卡** — 交付承诺机制拦下"PPT 式"渲染；合成前校验避免崩坏方案浪费 GPU；渲染后强制自审（ffprobe + 抽帧 + 音频分析），垃圾片出不了厂。每次供应商选择、风格决定和降级方案都进可审计的决策日志
- **内置预算管控** — 事前预估、花费上限、单笔行动审批阈值。账单不会给你惊喜
...
> **各家开通方法、价格与免费额度，见完整指南：** [`docs/PROVIDERS.md`](docs/PROVIDERS.md)
...
- **锁定**预算 — 调用前圈住资金
- 事后**结算** — 落账实际开销
- **可配置模式** — `observe`（只记账）、`warn`（超支报警）、`cap`（硬顶）
- **按行动审批** — 超过阈值（默认 0.50 美元）的单笔支出暂停待确认
- **总预算上限** — 默认 10 美元，完全可调
...
> **规划中：** 接入 **Ollama** 与 **LM Studio** 的本地 LLM 支持——整条生产流水线不依赖任何云端大模型也能跑。
